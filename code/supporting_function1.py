...
class CascadeInpaintingNet(nn.Module):
    ...
    def forward(self, input_imgs, he_imgs, h_lines_list, v_lines_list, roi_boxes_list):
        ...
        pad_size = 8
        patches_input = roi_align(input_imgs, cat_expanded_rois, output_size=(pad_size, pad_size), spatial_scale=1.0,
                                  sampling_ratio=4)
        patches_he = roi_align(he_imgs, cat_expanded_rois, output_size=(pad_size, pad_size), spatial_scale=1.0,
                               sampling_ratio=4)
...
def two_stage_alignment(he_path, params1_path, params2_path, target_path, output_path, save_stage1=True):
    ...
    return stage2
def simulate_low_resolution_sampling(matrix: np.ndarray,
    ...
    if psf_sigma is None:
        psf_sigma = float(sampling_interval) / 2.0
    blurred = gaussian_filter(mat, sigma=psf_sigma, mode="reflect")
    valid = (mat > 0).astype(np.float32)
    h, w = mat.shape
    out_h = max(1, int(np.round(h / sampling_interval)))
    out_w = max(1, int(np.round(w / sampling_interval)))
    def integral_image(a: np.ndarray) -> np.ndarray:
        ...
        return I
    I_img = integral_image(blurred)
    I_msk = integral_image(valid)
    y0 = (np.arange(out_h, dtype=np.float32) * sampling_interval).reshape(-1, 1)
    y1 = y0 + sampling_interval
    x0 = (np.arange(out_w, dtype=np.float32) * sampling_interval).reshape(1, -1)
    x1 = x0 + sampling_interval
    y0 = np.clip(y0, 0.0, float(h))
    y1 = np.clip(y1, 0.0, float(h))
    x0 = np.clip(x0, 0.0, float(w))
    x1 = np.clip(x1, 0.0, float(w))
    def integral_bilinear(I: np.ndarray, y: np.ndarray, x: np.ndarray) -> np.ndarray:
        y0i = np.floor(y).astype(np.int32)
        x0i = np.floor(x).astype(np.int32)
        y1i = np.clip(y0i + 1, 0, h)
        x1i = np.clip(x0i + 1, 0, w)
        wy = (y - y0i).astype(np.float32)
        wx = (x - x0i).astype(np.float32)
        Ia = I[y0i, x0i]
        Ib = I[y0i, x1i]
        Ic = I[y1i, x0i]
        Id = I[y1i, x1i]
        return (1 - wy) * (1 - wx) * Ia + (1 - wy) * wx * Ib + wy * (1 - wx) * Ic + wy * wx * Id
    def rect_sum(I: np.ndarray, ya: np.ndarray, xa: np.ndarray, yb: np.ndarray, xb: np.ndarray) -> np.ndarray:
        return (integral_bilinear(I, yb, xb)
                - integral_bilinear(I, ya, xb)
                - integral_bilinear(I, yb, xa)
                + integral_bilinear(I, ya, xa))
    sum_img = rect_sum(I_img, y0, x0, y1, x1)
    sum_msk = rect_sum(I_msk, y0, x0, y1, x1)
    area = (y1 - y0) * (x1 - x0)
    area = np.maximum(area, eps)
    proc_matrix = (sum_img / area).astype(np.float32)
    proc_matrix[sum_msk <= eps] = 0.0
    return proc_matrix
def process_msi_file_LR(input_file: str,
    ...
    for identifier, flat_data in tqdm(tasks, desc="Processing", unit="img"):
        ...
        for is_sampling in [False, True]:
            if is_sampling:
                ...
                proc_matrix = simulate_low_resolution_sampling(matrix, sampling_interval)
                suffix = "sampling"
