...
def train_pipeline():
    save_dir = Path("/mnt/xhdisk/02 MSI/SRP/【监控】欠采样")
    save_dir.mkdir(parents=True, exist_ok=True)
    linux_root_dir = r"/mnt/xhdisk/02 MSI/SRP/SSR"
    csv_file_path = r"/mnt/xhdisk/02 MSI/SRP/SSR/train_data.csv"
    TOTAL_EPOCHS = 100
    WARMUP_EPOCHS = 3
    ACCUM_STEPS = 8
    BATCH_SIZE = 1
    ...
    optimizer_G = torch.optim.AdamW(
        ...
        lr=4e-4, weight_decay=1e-3, betas=(0.5, 0.999)
    )
    optimizer_D = torch.optim.AdamW(
        discriminator.parameters(),
        lr=2e-4, weight_decay=1e-3, betas=(0.5, 0.999)
    )
